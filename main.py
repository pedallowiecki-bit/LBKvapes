...
async def setup(interaction: discord.Interaction):
    ...
    await guild.create_text_channel("👋┃powitania", category=c_lob)
    ...
    await guild.create_text_channel("📑┃zasady-ticketow", category=c_hlp)
    await guild.create_voice_channel("❓ ‧ OFF (15-21)", category=c_hlp)
    await guild.create_voice_channel("🚨 ‧ SPRAWDZANIE", category=c_hlp)
    await guild.create_voice_channel("⚠️ ‧ WYTŁUMACZENIE", category=c_hlp)
    ...
    await guild.create_text_channel("📸┃zrzuty-ekranu", category=c_med)

    # --- KATEGORIA: CHATY ---
    c_cha = await guild.create_category("MINT.MC - CHATY")
    await guild.create_text_channel("💬┃chat-ogolny", category=c_cha)
    await guild.create_text_channel("⛏️┃screeny-z-gry", category=c_cha)
    await guild.create_text_channel("🤖┃komendy", category=c_cha)

    # 3. UPRAWNIENIA (Tylko ranga Gracz widzi resztę)
    for cat in [c_lob, c_hos, c_inf, c_hlp, c_med, c_cha]:
        await cat.set_permissions(everyone, read_messages=False)
        await cat.set_permissions(created_roles["🟢 GRACZ"], read_messages=True)

    await interaction.followup.send("🔥 Serwer Mint.mc został pomyślnie zbudowany!", ephemeral=True)

# --- URUCHOMIENIE ---
if __name__ == "__main__":
    keep_alive() # Odpala Flask, żeby Render nie wyłączył bota
    try:
        bot.run(TOKEN)
    except discord.errors.LoginFailure:
        print("❌ BŁĄD: Token jest nieprawidłowy!")
